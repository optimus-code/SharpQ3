	...
	*/
	public static class win_glimp
	{
		...
		*/
		...
		static bool GLW_SetPixelFormat(HDC hdc, PIXELFORMATDESCRIPTOR *pPFD, int colorbits, int depthbits, int stencilbits, bool stereo) {
		...
		static bool GLW_InitDriver(HWND hwnd) {
		    ...
		    if (!GLW_SetPixelFormat(gl_hdc, &pfd, colorbits, depthbits, stencilbits, (bool)r_stereo->integer)) {
		    ...
		    glConfig.stereoEnabled = (pfd.dwFlags & PFD_STEREO) ? true : false;
			return true;
		}

		static HWND create_main_window(int width, int height, bool fullscreen)
		{
			//
		...
		static void SetMode(int mode, bool fullscreen) {
			if (fullscreen) {
		...
		*/
		void GLimp_EndFrame (void)
		{
			...
			// check logging
			QGL_EnableLogging( (bool) r_logFile->integer );
		}
		...
		*/
		void GLimp_Init( void )
		{
			...
			SetMode(r_mode->integer, (bool)r_fullscreen->integer);

			if (get_render_api() == RENDER_API_GL) {
				g_wv.hWnd_opengl = create_main_window(glConfig.vidWidth, glConfig.vidHeight, (bool)r_fullscreen->integer);
		...
		*/
		...
		void vk_imp_init() {
			...
			if (get_render_api() == RENDER_API_VK) {
				g_wv.hWnd_vulkan = create_main_window(glConfig.vidWidth, glConfig.vidHeight, (bool)r_fullscreen->integer);
		...
		void dx_imp_init() {
			...
			if (get_render_api() == RENDER_API_DX) {
				g_wv.hWnd_dx = create_main_window(glConfig.vidWidth, glConfig.vidHeight, (bool)r_fullscreen->integer);
